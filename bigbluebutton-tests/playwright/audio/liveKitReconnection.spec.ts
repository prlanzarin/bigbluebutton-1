...
test.describe('LiveKit reconnection', { tag: ['@long-running', '@media'] }, () => {
  test.beforeEach(() => {
  ...
  test('a user ejected by the user-left sweep is heard again after rejoining', async ({ browser }, testInfo) => {
    ...
    expect(
      fixture.viewerLogLines.filter((line) => line.includes('LiveKit: setSenderTrackEnabled failed')),
      'no publish should have hit a torn-down room',
    ).toEqual([]);
  ...
  test('a full reconnect while unmuted keeps the mic and honours a later mute', async ({ browser }, testInfo) => {
    ...
    expect(
      fixture.viewerLogLines.filter((line) => line.includes('LiveKit: fatal audio publish error detected')),
      'no publish should have been escalated into a forced reconnect',
    ).toEqual([]);
