...
// Past the room's 60s stall bound, so the sampling covers the retry campaign the
// stall fix starts and not just the SDK's own reconnect attempts. The window the
// client is most likely to leave silent is the one after it begins recovering.
const REFUSED_OUTAGE_TIME = 95_000;
...
// app.video.mediaTimedOutError. Asserting the text keeps an unrelated toast from
// standing in for the notice these cases exist to check.
const CAMERA_LOST_NOTICE = 'Your webcam stream has been interrupted';

test.describe('LiveKit media outage', { tag: ['@long-running', '@media'] }, () => {
  test.beforeEach(() => {
  ...
  test('an open microphone with nothing behind it is not left unannounced', async ({ browser }, testInfo) => {
    test.setTimeout(ELEMENT_WAIT_EXTRA_LONG_TIME * 20);
    ...
    // Every signal socket is refused, the ones a reconnect opens included, so
    // the room stays down through its whole retry campaign instead of healing
    // partway and hiding the window this asserts on. GraphQL is untouched.
    await fixture.cutLiveKitSignal();
    const interruptedAs = await waitForMediaInterrupted(viewerPage.page);
    const samples = await sampleMediaOutage(viewerPage.page, REFUSED_OUTAGE_TIME);
    fixture.restoreLiveKitSignal();
    ...
    // And whatever the user was told, the client must not offer to open a
    // microphone that has no session to carry it. Closing one is deliberately
    // not asserted on: that has to stay available for the whole outage.
    const offeredToUnmute = samples.filter((sample) => sample.mediaConnected === false && sample.canUnmute);
    expect(offeredToUnmute, 'a microphone with no media session must not be offered for unmuting').toEqual([]);
  });
  ...
  test('media comes back when the network does, or the user can rejoin', async ({ browser }, testInfo) => {
    ...
    // The voice record is re-created when the session comes back, and whether
    // the server re-applies its mute-on-start before this point is a race, so
    // the end state is not fixed. What recovery owes either way is a session
    // that agrees with its UI and can carry audio again.
  ...
  test('a webcam survives an eject and rejoin, or the user is told it did not', async ({ browser }, testInfo) => {
    ...
    const wasTold = toasts.some((text) => text.includes(CAMERA_LOST_NOTICE));
