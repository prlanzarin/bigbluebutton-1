...
class AudioSettings extends React.Component {
  ...
  handleConfirmationClick() {
    ...
    if (isConnected) liveChangeInputDevice(inputDeviceId).then(this.unmuteOnExit);
  ...
  unmuteOnExit() {
    const { isMuted, toggleMuteMicrophoneSystem, toggleVoice } = this.props;
    const { unmuteOnExit } = this.state;

    // Check for isMuted as well because there are other actions that can
    // change mute state. Only revert if needed.
    if (unmuteOnExit && isMuted) {
  ...
  changeInputDevice(deviceId) {
    ...
    if (isConnected) {
      return liveChangeInputDevice(deviceId);
    }

    return changeInputDevice(deviceId);
  }
