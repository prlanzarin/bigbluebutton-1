...
trait JoinMediaGroupReqMsgHdlr extends RightsManagementTrait {
  ...
  def handle(msg: JoinMediaGroupReqMsg, state: MeetingState2x,
    ...
    val participantTypeValid = MediaGroupApp.isParticipantTypeValid(msg.body.participantType)
    ...
    MediaGroupApp.findMediaGroup(groupId, state.mediaGroups) match {
      case Some(mg) =>
        if (!participantTypeValid || (mg.locked && permissionFailed(
          ...
          mg.findParticipant(participantUserId) match {
            ...
            case None =>
              val updatedGroups = MediaGroupApp.addMediaGroupParticipant(
                groupId,
                participant,
                msg.body.participantType,
                state.mediaGroups
              )
              broadcastEvent(mg)
              MediaGroupUserDAO.insertUser(liveMeeting.props.meetingProp.intId, groupId, participant.userId, msg.body.participantType, participant.active)
