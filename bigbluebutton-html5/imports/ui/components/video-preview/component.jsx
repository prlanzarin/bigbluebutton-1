...
class VideoPreview extends Component {
    ...
    if (deviceInfo.hasMediaDevices) {
      try {
        ...
        navigator.mediaDevices.enumerateDevices().then((devices) => {
          ...
          let { webcams, areLabelled } = Service.digestVideoDevices(devices, webcamDeviceId);
            ...
            this.displayInitialPreview(webcams[0].deviceId)
              .then(async () => {
                if (!areLabelled) {
                  ...
                  webcams = Service.digestVideoDevices(newDevices, webcamDeviceId).webcams;
                }
  ...
  componentWillUnmount() {
    if (this.deviceStream) {
      Service.stopStreamTracks(this.deviceStream);
      this.deviceStream = null;
    }
  ...
  handleSelectProfile(event) {
    ...
    Service.changeProfile(selectedProfile.id);

    Service.applyProfileConstraints(this.deviceStream, selectedProfile).then(() => {
      // Late GUM resolution, clean up tracks, stop.
      if (!this._isMounted) return Service.stopStreamTracks(this.deviceStream);
      const track = this.deviceStream.getVideoTracks()[0];
  ...
  handleStartSharing() {
    ...
    if(!Service.storeStream(webcamDeviceId, this.deviceStream)) {
      Service.stopStreamTracks(this.deviceStream);
    }
    this.deviceStream = null;
  ...
  handleStopSharing() {
    ...
    const { webcamDeviceId } = this.state;
    Service.deleteStream(webcamDeviceId);
  ...
  displayPreview(deviceId, profile) {
    ...
    Service.changeProfile(profile.id);
    ...
    return this.doGUM(deviceId, profile).then((stream) => {
      // Late GUM resolution, clean up tracks, stop.
      if (!this._isMounted) return Service.stopStreamTracks(stream);
