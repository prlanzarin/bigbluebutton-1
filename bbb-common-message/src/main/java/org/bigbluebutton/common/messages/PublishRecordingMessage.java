...
public class PublishRecordingMessage implements IBigBlueButtonMessage {
	...
	public String toJson() {
		...
		HashMap<String, Object> header = MessageBuilder.buildHeader(PUBLISH_RECORDING, VERSION, null);
		return MessageBuilder.buildJson(header, payload);
	}

	public static PublishRecordingMessage fromJson(String message) {
		...
		if (obj.has("header") && obj.has("payload")) {
			...
			if (header.has("name")) {
				...
				if (PUBLISH_RECORDING.equals(messageName)) {
					if (payload.has(Constants.RECORD_ID)
						...
						return new PublishRecordingMessage(recordId, meetingId, externalMeetingId, format);
					}
				}
			}
		}
