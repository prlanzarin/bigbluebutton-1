      ...
      MediaGroupApp.findMediaGroup(groupId, state.mediaGroups) match {
        case Some(mg) =>
          ...
          senders.foreach { sender =>
            // FIXME participant type
            MediaGroupUserDAO.insert(liveMeeting.props.meetingProp.intId, mg.id, sender, MediaGroupParticipantType.SEND_ONLY)
          }
          receivers.foreach { receiver =>
            // FIXME participant type
            MediaGroupUserDAO.insert(liveMeeting.props.meetingProp.intId, mg.id, receiver, MediaGroupParticipantType.RECV_ONLY)
          }
        ...
        case None =>
          ...
          for (participant <- mg.findAllParticipants()) {
            // FIXME participant type
            MediaGroupUserDAO.insert(liveMeeting.props.meetingProp.intId, mg.id, participant, MediaGroupParticipantType.SEND_RECV)
          }
