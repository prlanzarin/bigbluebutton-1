...
object MediaGroupApp {
  ...
  def addMediaGroupParticipant(
    ...
    mediaGroups.find(id) match {
      case Some(mg) =>
        val updatedMg = participantType match {
          case MediaGroupParticipantType.SEND_ONLY =>
            mg.addSender(participant)
          case MediaGroupParticipantType.RECV_ONLY =>
            mg.addReceiver(participant)
          case MediaGroupParticipantType.SEND_RECV =>
            mg.addSender(participant).addReceiver(participant)
          case _ =>
            mg
        }
        mediaGroups.update(updatedMg)
  ...
  def isParticipantTypeValid(participantType: String): Boolean = {
    participantType match {
      case MediaGroupParticipantType.SEND_ONLY | MediaGroupParticipantType.RECV_ONLY | MediaGroupParticipantType.SEND_RECV =>
        true
      case _ =>
        false
    }
